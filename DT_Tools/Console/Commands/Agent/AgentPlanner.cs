...
{
    /// <summary>
    /// 任务规划器。原样自 AgentCommand.Planner 拆出，四条 Collect 链体位于独立文件
    /// （AgentDeliveryChain / AgentVacuumChain / AgentInstantChain / AgentGenerateChain）。
    ///
    /// PlanAll 现在返回并保留完整有序步骤列表（原来只在内部用，外部只取 [0]）。
    /// Runner 借此在同一 tick 内批量执行：所有 ChangesHand==false 的步骤（互不干扰，
    /// 可全部执行）+ 优先级最高的 1 个 ChangesHand==true 的步骤（同 tick 最多 1 次，
    /// 依据见 AgentStep.ChangesHand 上的注释）。
    /// </summary>
    internal static class AgentPlanner
    {
        ...
        /// <summary>单步版本，保留兼容（未使用多步执行的调用方可继续用它）。</summary>
